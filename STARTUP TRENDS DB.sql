CREATE DATABASE IF NOT EXISTS startup_trends;
USE startup_trends;
...
-- 5. Maximum amount raised during Seed stage
SELECT MAX(raised)
FROM startups
WHERE stage = 'Seed';

-- 6. Oldest company founded
SELECT MIN(founded)
FROM startups;

-- 7. Average valuation of all startups
...
-- 9. Average valuation by category rounded to 2 decimals
SELECT category, ROUND(AVG(valuation), 2) AS average_valuation
FROM startups
GROUP BY category;

-- 10. Average valuation by category ordered highest to lowest
SELECT category, ROUND(AVG(valuation), 2) AS average_valuation
FROM startups
GROUP BY category
ORDER BY average_valuation DESC;

-- 11. Number of companies in each category
...
-- 13. Average startup size by location
SELECT location, AVG(employees) AS average_size
FROM startups
GROUP BY location;

-- 14. Locations where average startup size is greater than 500
SELECT location, AVG(employees) AS average_size
FROM startups
GROUP BY location
